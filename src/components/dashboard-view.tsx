...
export default function DashboardView({ transactions, session }: DashboardViewProps) {
    ...
    const RECON_RULES: Record<string, {
        ...
        mapRow?: (cleanRow: any, rule: any) => { date: string, amount: number, txId: string, status: string } | null
    }> = {
        '一般信用卡': {
        ...
        '掃碼-街口支付': {
            ...
            mapRow: (row, rule) => {
                ...
                const status = String(findVal(rule.statusFields) || '成功').trim();
        ...
        '電子票證-悠遊卡-小額': {
            ...
            mapRow: (row, rule) => {
                ...
                const amount = Number(findVal(rule.amountFields) || 0);
                const txId = String(findVal(rule.idFields) || '-');
                const status = String(findVal(rule.statusFields) || '交易成功').trim();
                ...
                const parsed = data.map((row: any) => {
                    ...
                    Object.entries(row).forEach(([k, v]) => {
                        ...
                        if (typeof v === 'string') {
                            ...
                            // Any non-amount integer larger than 10000 is likely an ID or RRN
                            // but small amounts like 498 should remain as numbers if they are amounts.
                            // However, sheet_to_json doesn't know which is what yet.
                            // We use a heuristic: if it's an integer and very large, it's an ID.
                    ...
                    // 2. Use Independent Mapper if available
                    if (rule.mapRow) {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card title="交易筆數"
                    value={`${stats.totalTx.toLocaleString()} 筆`}
                    sub={`平均單筆: $${stats.totalTx > 0 ? Math.round(stats.totalRevenue / stats.totalTx).toLocaleString() : 0}`}
                    icon={<Ticket className="w-8 h-8 text-indigo-500 opacity-80" />} />
            ...
            {/* Reconciliation Tab */}
            {
                activeTab === 'reconciliation' && (
                    ...
                    `}} />
                        ...
                        {/* AI CFO Chatbot */}
                        {/* AiCfoChat commented out for debugging */}
                        <div className="hidden">AiCfoChat Placeholder</div>
                    </div>
                );
}
